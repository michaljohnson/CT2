...
#define START_ADDRESS (*((volatile uint8_t*)(0x64000000)))
...
int main(void)
{
    ...
    /// STUDENTS: To be programmed
		...
		for(int i = 0; i < NR_OF_DATA_LINES; i++){
			uint8_t pattern = 1 << i;
			START_ADDRESS = pattern;
			if (!(START_ADDRESS == pattern)){
				CT_LED->BYTE.LED7_0 |= pattern;
			}
		}
    ...
    /// STUDENTS: To be programmed
		...
		while (address) {
			address >>= 1;
			uint32_t addr_offset = 0x64000000 + address;
			volatile uint8_t *pointer_addr_offset = ((volatile uint8_t*)(addr_offset));
			
			*pointer_addr_offset = 0xAA;
		}
		...
		while (address) {
			...
			*pointer_addr_offset = 0x55;
			
			uint16_t addressIn = (uint16_t) 0x01 << NR_OF_ADDRESS_LINES;
			while (addressIn) {
				addressIn >>= 1;
				uint32_t addr_offsetIn = 0x64000000 + addressIn;
				volatile uint8_t *pointer_addr_offsetIn = ((volatile uint8_t*)(addr_offsetIn));
				
				if(addr_offset == addr_offsetIn){
					if (*pointer_addr_offsetIn != *pointer_addr_offset){
						CT_LED->HWORD.LED31_16 |= address;
						if (addressIn == 0x00){
							CT_LED->HWORD.LED31_16 |= 0xF000;
						}
					}
				} else if(!(*pointer_addr_offsetIn == 0xAA)){
					CT_LED->HWORD.LED31_16 |= address;
					if (addressIn == 0x00){
						CT_LED->HWORD.LED31_16 |= 0xF000;
					}
